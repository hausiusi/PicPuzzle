...
class DragDrop:
    def __init__(self, group_textures):
    ...
    def _drop_callback(self, sender):
        print(f"Dropped from {self._drag_object} into {sender}")
        dpg.configure_item(self._drag_object, texture_tag=dpg.get_item_configuration(sender)["texture_tag"])
        dpg.configure_item(sender, texture_tag=self._drag_texture)

    def create_draggable_image(self, image_data, width, height, image_tag, texture_tag, parent):
        if dpg.does_item_exist(parent):
            if not dpg.does_item_exist(texture_tag):
                with dpg.texture_registry():
                    texture_tag = dpg.add_static_texture(width, height, image_data, tag=texture_tag)
            ...
            img_btn = dpg.add_image_button(texture_tag, tag=image_tag, payload_type="image_payload",
